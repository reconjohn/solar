...
MLE estimates the function then, likelihood ratio (LR) test examines SDM to SAR (if $\theta$ = 0), SEM (if $\theta$ = -$\rho\beta$), or OLS. If spatial lagged variables are added to the fuction due to being significant, PV uptake has been enhanced by network effects (spillovers). 

* Spatial Durbin model (having both SAR and SEM)
* Spatial autoregressive model (SAR)
* Spatial error model (SEM)

Moran's I for tendency toward clustering (1) or dispersion (-1). 
...
> Energy justice in terms of distributional, recognition and procedural in production and consumption regarding evaluative and normative reach. Futher suggesting non-activist origins of energy justice, engaging with economics, and uniting systems of production and consumption. What, who and how. 
